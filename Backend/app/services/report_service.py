...
def generate_report(assessment_id: str, calc: object, 
    ...
    # Reconstruct ages, years to ret, and expenses
    from app.core.formulas import excel_PV, excel_PMT, BEGIN, END, real_rate, real_rate_monthly
    rates_dict = {
        "inflation_post": 0.06,
        "roi_post": 0.08,
        "inflation_pre": 0.06,
    ...
    rr = real_rate(rates_dict["roi_post"], rates_dict["inflation_post"])
    rr_mly = real_rate_monthly(rr)

    client_ret_age = personal.client_retirement_age or 60
    client_age = personal.client_age or 30
    client_yrs_to_ret = max(0, client_ret_age - client_age)
    client_ret_period = 80 - client_ret_age
    
    if calc.client_corpus:
        client_exp_at_ret = abs(excel_PMT(rr_mly, client_ret_period * 12, -calc.client_corpus, 0, BEGIN))
        client_exp_today = abs(excel_PV(rates_dict["inflation_pre"], client_yrs_to_ret, 0, -client_exp_at_ret, END))
    else:
        client_exp_at_ret = 0.0
        client_exp_today = 0.0

    spouse_ret_age = personal.spouse_retirement_age or 55
    spouse_age = personal.spouse_age
    if spouse_age is not None:
        spouse_yrs_to_ret = max(0, spouse_ret_age - spouse_age)
        spouse_ret_period = 80 - spouse_ret_age
        if calc.spouse_corpus:
            spouse_exp_at_ret = abs(excel_PMT(rr_mly, spouse_ret_period * 12, -calc.spouse_corpus, 0, BEGIN))
            spouse_exp_today = abs(excel_PV(rates_dict["inflation_pre"], spouse_yrs_to_ret, 0, -spouse_exp_at_ret, END))
        else:
            spouse_exp_at_ret = 0.0
            spouse_exp_today = 0.0
    else:
        spouse_yrs_to_ret = 0
        spouse_exp_at_ret = 0.0
        spouse_exp_today = 0.0
    ...
    replacements = {
        ...
        "client_years_to_retirement": str(client_yrs_to_ret),
        "client_expense_today":      fmt_inr(client_exp_today),
        "client_expense_at_retirement": fmt_inr(client_exp_at_ret),
        ...
        "spouse_retirement_age":     str(spouse_ret_age),
        "spouse_years_to_retirement": str(spouse_yrs_to_ret),
        "spouse_expense_today":      fmt_inr(spouse_exp_today),
        "spouse_expense_at_retirement": fmt_inr(spouse_exp_at_ret),
